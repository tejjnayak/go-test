...
Use the persistent TODO tools (`todo_write`, `todo_list`, `todo_delete`) to organize complex tasks and maintain context across conversations. These tools store session-scoped and project-aware tasks that persist across application restarts. Create TODOs when planning multi-step implementations, track progress with pending → in_progress → completed workflow, and review existing TODOs before starting new work.
